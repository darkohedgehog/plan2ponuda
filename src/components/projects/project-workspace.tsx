  ...
  return (
    <main className="flex flex-col gap-6">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <dl className="mt-6 grid gap-3 border-t border-slate-200 pt-6 sm:grid-cols-2 xl:grid-cols-4">
          <SummaryItem label="Client" value={project.clientName ?? "Not set"} />
          <SummaryItem
            label="Object type"
            value={formatObjectType(project.objectType)}
          />
          <SummaryItem label="Area" value={`${project.areaM2} m2`} />
          <SummaryItem
            label="Floor plan"
...
function WorkflowStepCard({ index, step }: WorkflowStepCardProps) {
  return (
    <article className="grid gap-4 rounded-md border border-slate-200 bg-slate-50 p-4 md:grid-cols-[auto_1fr_auto] md:items-center">
      <div className="flex h-9 w-9 items-center justify-center rounded-md bg-white text-sm font-semibold text-blue-700 ring-1 ring-slate-200">
        {index + 1}
      </div>
      <div>
        <div className="flex flex-wrap items-center gap-3">
      ...
      {step.actionHref ? (
        ...
        <span className="inline-flex h-10 items-center justify-center rounded-md border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-400">
          {step.actionLabel}
        </span>
      )}
    </article>
...
function WorkflowStateBadge({ state }: WorkflowStateBadgeProps) {
  ...
  return (
    <span
      className={`inline-flex rounded-md px-2.5 py-1 text-xs font-semibold capitalize ring-1 ring-inset ${stateStyles[state]}`}
    >
      {state}
    </span>
  );
