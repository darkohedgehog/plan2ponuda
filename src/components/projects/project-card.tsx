...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <article className="grid min-w-0 gap-4 bg-white p-4 transition-colors hover:bg-frosted-blue-50/70 sm:p-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(8rem,0.95fr)_minmax(6rem,0.7fr)_minmax(7rem,0.65fr)_auto] lg:items-center lg:gap-5">
      <div className="min-w-0">
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between lg:justify-start">
          <h3 className="min-w-0 truncate text-base font-semibold text-deep-twilight-950">
            {project.name}
          </h3>
          <ProjectStatusBadge status={project.status} />
        </div>
        <p className="mt-2 truncate text-sm text-deep-twilight-700/70">
...
function MetadataItem({ label, value }: MetadataItemProps) {
  return (
    <div className="min-w-0 rounded-md bg-frosted-blue-50 px-3 py-2 ring-1 ring-frosted-blue-100 lg:bg-transparent lg:p-0 lg:ring-0">
      <p className="text-xs font-medium uppercase tracking-wide text-deep-twilight-700/55">
        {label}
      </p>
      <p className="mt-1 truncate text-sm font-medium text-deep-twilight-800">
        {value}
      </p>
    </div>
  );
